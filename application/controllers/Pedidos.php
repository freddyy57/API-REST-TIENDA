...
class Pedidos extends REST_Controller {
  ...
  public function realizar_orden_post( $token = "0", $id_usuario = "0" ) {
    ...
    if( !isset( $data["items"]) || strlen( $data['items']) == 0 ) {
        $respuesta = array(
                       'error' => TRUE,
                       'mensaje' => " Faltan los items en el pedido"
                     );
        $this->response( $respuesta, REST_Controller::HTTP_BAD_REQUEST );
        return;
    }
    ...
    $condiciones = array('id' => $id_usuario,
                          'token' => $token
                        );
    $this->db->where( $condiciones );
    // comprobar si el id y token coinciden en la db
    ...
    // TOKEN Y USUARIO VALIDO
    // Resetear Query
    $this->db->reset_query();

    $insertar = array( 'usuario_id' => $id_usuario );
    // insertar usuario_id en la tabla rdenes
    $this->db->insert( 'ordenes', $insertar );
    // obtener id de la orden que acabamos de crear
    // regresa id de la ultima incersion
    ...
    foreach( $items as &$producto_id ) {
      // TODO aquí deberiamos validar si el producto_id es vlaido
  ...
  public function borrar_pedido_delete( $token = "0", $id_usuario = "0", $orden_id = "0") {
    ...
    if( $token == "0" || $id_usuario == "0" || $orden_id == 0) {
      $respuesta = array(
                     'error' => TRUE,
                     'mensaje' => "Token y/o usuario y/o pedido invalido"
                   );
      $this->response( $respuesta, REST_Controller::HTTP_BAD_REQUEST );
      return;
    }
    ...
    // Verificar si la orden pertenece a ese usuario
